...
class ModelWeidianCategory extends Model {
	...
	public function getCategories($data = array()) {
		$sql = "SELECT cp.category_id AS category_id, c1.sent, GROUP_CONCAT(cd1.name ORDER BY cp.level SEPARATOR '&nbsp;&nbsp;&gt;&nbsp;&nbsp;') AS name, c1.parent_id, c1.sort_order FROM " . DB_PREFIX . "category_path cp LEFT JOIN " . DB_PREFIX . "category c1 ON (cp.category_id = c1.category_id) LEFT JOIN " . DB_PREFIX . "category c2 ON (cp.path_id = c2.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd1 ON (cp.path_id = cd1.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd2 ON (cp.category_id = cd2.category_id) WHERE cd1.language_id = '" . (int)$this->config->get('config_language_id') . "' AND cd2.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function updateCategoryProductTotal($product_total, $category_id) {

		$this->db->query("UPDATE " . DB_PREFIX . "category SET product_total = '" . (int)$product_total . "' WHERE category_id = '" . (int)$category_id . "'");

	}
	
	
	public function updateSent($category_id, $weidian_category_id) {
		
		$this->db->query("UPDATE " . DB_PREFIX . "category SET sent = 1, weidian_category_id = '" . (int)$weidian_category_id . "' WHERE category_id = '" . (int)$category_id . "'");
		
	}
	
	public function deleteSent($category_id, $weidian_category_id) {
		
		$this->db->query("UPDATE " . DB_PREFIX . "category SET sent = 0, weidian_category_id = 0 WHERE category_id = '" . (int)$category_id . "'");
